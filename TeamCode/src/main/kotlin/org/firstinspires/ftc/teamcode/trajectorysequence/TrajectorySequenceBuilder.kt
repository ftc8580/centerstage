...
class TrajectorySequenceBuilder(
    ...
    private fun projectGlobalMarkersToLocalSegments(
        ...
        markers.sortedBy(TrajectoryMarker::time)
        var segmentIndex = 0
        var currentTime = 0.0
        for ((markerTime, callback) in markers) {
            var segment: SequenceSegment? = null
            var segmentOffsetTime = 0.0
            while (segmentIndex < sequenceSegments.size) {
                val seg = sequenceSegments[segmentIndex]
            ...
            if (segmentIndex >= sequenceSegments.size) {
                segment = sequenceSegments[sequenceSegments.size - 1]
                segmentOffsetTime = segment.duration
            }
            val newSegment: SequenceSegment? = when (segment) {
                is WaitSegment -> {
                    ...
                    WaitSegment(segment.startPose, segment.duration, newMarkers)
                }
                ...
                is TrajectorySegment -> {
                    val newMarkers: MutableList<TrajectoryMarker> = ArrayList(segment.trajectory.markers)
                    newMarkers.add(TrajectoryMarker(segmentOffsetTime, callback))
                    TrajectorySegment(
                        Trajectory(
    ...
    private fun displacementToTime(sequenceSegments: List<SequenceSegment?>, s: Double): Double {
        ...
        for (segment in sequenceSegments) {
            if (segment is TrajectorySegment) {
                ...
                currentTime += segment?.duration ?: 0.0
            }
        }
        return 0.0
    }
